using PenDynamicsLab.Paint;
using Xunit;

namespace PenDynamicsLab.Tests;

/// <summary>
/// Pins the property issue 73 exists for: <b>zoom and pan move the view, never the ink</b>.
/// </summary>
/// <remarks>
/// <para>
/// The stroke tabs have no mapping to get wrong — the control is the canvas, so a position in one
/// is a position in the other. Paint puts a document behind a viewport, and every coordinate now
/// passes through a magnification and an offset. That is the part worth holding down, and it is
/// held down here rather than by drawing and looking, because a mapping error of half a pixel is
/// invisible on screen and ruinous in a recording.
/// </para>
/// <para>
...
public class PaintViewportTests
{
    ...
    {
        // The acceptance property from issue 73. The same document position is the same document
    ...
    {
        // Issue 73 names this case because it is the one a cached origin gets wrong. Two samples
